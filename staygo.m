function etimes = staygo(data,index)
    ...
    rlabels = find(contains(data.EVENTS.LABELS, 'REACH'));          %Get Label indices of all reaches performed during trial
    rtimes = round(data.EVENTS.TIMES(rlabels)*1000);                %Look up time each event code was produced & convert to samples
    
    vcomp = [data.Right_HandXVel(go:rtimes(1)),data.Right_HandYVel(go:rtimes(1))];      % X and Y components of velocity
    vel = sqrt(sum(vcomp.^2,2));                                                        % compute total velocity
